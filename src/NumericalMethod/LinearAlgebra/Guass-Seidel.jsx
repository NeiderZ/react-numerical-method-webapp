...
function Guass_Seidel() {
    ...
    function cal(){
        ...
        do{
            for(let j = 0;j < n;j++){
                let temp = res[j]
                for(let k = 0;k < n;k++){
                    if(j !== k) temp -= a[j][k] * xnew[k];
                }
                xnew[j] = temp / a[j][j]
                error[j] = Math.abs((xnew[j] - xold[j]) / xnew[j])
            }
    ...
    return (
        <div>
            <h2>Guass-Seidel Iteration Method</h2>
                ...
                <thead>
                    <tr>
                        ...
                        {data.length > 0 && data[0].x.map((_, idx) => (
                            <th key={"x"+idx}>x<sub>{idx + 1}</sub></th>
                        ))}
                        {data.length > 0 && data[0].e.map((_, idx) => (
                            <th key={"e"+idx}>error<sub>{idx + 1}</sub></th>
                        ))}
                    </tr>
                </thead>
                <tbody>
